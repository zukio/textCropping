...
from modules.communication.tcp_client import DelayedTCPSender, hello_server
...
async def main(args):
    try:
        # プロセスサーバのタスクを開始する
        server_task = asyncio.create_task(start_server(12321))
...
if __name__ == "__main__":
    ...
    if check_existing_instance(12321):
        print("既に起動しています。")
    ...
    # [TCP] DelayedUDPSenderのインスタンスを作成し、それをTargetFileHandlerのインスタンスとlist_files関数に渡します。
    ...
    event_handler = TargetFileHandler(
        args.exclude_subdirectories, tcp_sender, args.ip, args.port, args.seconds, output_dir)

    # サーバーとの通信を試みる
    response = hello_server(path)
    if response is not None:
        print("Hello TCP: " + response)
