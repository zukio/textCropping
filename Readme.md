...
テキスト認識の精度を向上させるために、以下の機能が追加されました：

### 1. 複数OCRエンジンのサポート

* `pytesseract` (デフォルト): 基本的なOCR機能
* `EasyOCR`: 高精度な多言語OCR (オプション)
* `PaddleOCR`: 中国Baiduが開発した高性能OCR (オプション)

### 2. 自動エンジン選択機能

* `config.json` で `ocr_engine` を設定可能:
  * `"auto"`: 利用可能な最適なエンジンを自動選択
  * `"tesseract"`: Tesseract OCRを使用
  * `"easyocr"`: EasyOCRを使用
  * `"paddleocr"`: PaddleOCRを使用
...
高精度OCRを使用するには、以下のコマンドで追加ライブラリをインストールしてください：

```bash
pip install easyocr paddlepaddle paddleocr
```

インストール後、アプリケーションを再起動すると自動的に高精度OCRが使用されます。
* 通知先 IP\:PORT は設定可
* JSON 形式 またはプレーンテキスト
* UDP機能は ON/OFF 切替可
